...
func healthyAssessment() domain.HealthAssessment {
	checkedAt := time.Date(2026, 9, 21, 12, 0, 0, 0, time.UTC)
	return domain.HealthAssessment{
		Status:    domain.ReadinessStatusReady,
		CheckedAt: checkedAt,
		Components: []domain.ComponentHealth{
			{Name: "database", Status: domain.ComponentStatusUp, CheckedAt: checkedAt},
			{Name: "rabbitmq", Status: domain.ComponentStatusUp, CheckedAt: checkedAt},
		},
	}
}
...
func TestHealthHandlerReadinessReturns503ForAFailedComponent(t *testing.T) {
	...
	assessment.Status = domain.ReadinessStatusNotReady
	assessment.Components[0].Status = domain.ComponentStatusDown
...
func TestHealthHandlerReadinessReturns503WhenAComponentIsUnknown(t *testing.T) {
	...
	assessment := healthyAssessment()
	assessment.Status = domain.ReadinessStatusNotReady
	assessment.Components[1].Status = domain.ComponentStatusUnknown
	assessor.EXPECT().Assess(gomock.Any()).Return(assessment)
	healthHandler := NewHealthHandler(assessor, nil)
...
func TestHealthHandlerReadinessOmitsRawDependencyErrors(t *testing.T) {
	...
	useCase := usecase.NewHealthUseCase([]ports.DependencyChecker{
