...
import (
	...
	"github.com/billykore/project-one/internal/featureflags/api/dto"
...
type FeatureFlagHandler struct {
	useCase     featureflagports.FeatureFlagUseCase
	validator   platformports.Validator
	environment string
}

// NewFeatureFlagHandler creates a feature-flag handler.
func NewFeatureFlagHandler(useCase featureflagports.FeatureFlagUseCase, validator platformports.Validator, environment string) *FeatureFlagHandler {
...
func (h *FeatureFlagHandler) SetEnvironment(c echo.Context) error {
	...
	setting, err := h.useCase.SetEnvironment(c.Request().Context(), user.Username, c.Param("key"), featureflagdomain.Environment(c.Param("environment")), featureflagdomain.AvailabilityMode(request.Mode), request.RolloutPercentage, request.Revision, request.Reason)
	if err != nil {
		return err
	}
...
func (h *FeatureFlagHandler) SetOverrides(c echo.Context) error {
	...
	if err := h.useCase.SetOverrides(c.Request().Context(), user.Username, c.Param("key"), featureflagdomain.Environment(request.Environment), request.Include, request.Exclude, request.Reason); err != nil {
		return err
	}
...
func flagResponse(flag featureflagdomain.FeatureFlag) dto.FeatureFlagResponse {
