# Development Requirement

Act as Senior Frontend Web Engineer to make a plan from this development requirements.

## Prerequiset

- Create new working branch with name  `<category:feature|fix|refactor|test|chore|docs>/<short-description`.

Read following document for informations about the project:

- @web/AGENTS.md.
- @web/README.md.
- @api/swagger/swagger.json.

## Planning

- Think step by step and outline a clear plan.
- List the main steps you would take.
- Call out important decisions or tradeoffs.
- Mention edge cases we should keep in mind.

## Development Objective

- Create loading.tsx for register page.
- The loading page should use skeleton loading like other page.
